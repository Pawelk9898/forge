...
function goToSegment(index) {
  ...
  //Scene.updateCutFloors(State.segments, index);
...
function updateOverlays(seg) {
  ...
  if (segInfo) segInfo.textContent =
  ...
  if (feed) feed.textContent = seg.is_cutting
    ? `F: ${seg.feed_rate.toFixed(0)} mm/min  S: ${seg.spindle_rpm.toFixed(0)} RPM`
    : 'RAPID';

  const warn = document.getElementById('ol-warn');
  if (warn) warn.textContent = seg.force_critical
    ? '🔴 CRITICAL FORCE' : seg.force_warning ? '⚠ WARNING FORCE' : '';
  ...
  if (fzEl) fzEl.textContent = seg.fz > 0
    ? `fz: ${seg.fz.toFixed(4)} mm/tooth` : 'fz: —';
}
...
function buildSegmentDots(segments) {
  ...
  segments.forEach((seg, i) => {
    ...
    dot.title     = `Seg ${i + 1} — ${seg.is_cutting ? `Fc=${seg.fc.toFixed(0)}N` : 'RAPID'}`;
    dot.onclick   = () => { stopPlay(); goToSegment(i); };
...
function updateTimeline(index) {
  ...
  document.querySelectorAll('.seg-dot').forEach((d, i) => {
...
function updateAlert(seg) {
  ...
  if (!banner) return;
  if (seg.force_critical) {
    text.textContent = `CRITICAL FORCE — SEGMENT ${seg.segment_index + 1} — ${seg.fc.toFixed(0)}N`;
...
function stepForward(n = 1) { stopPlay(); goToSegment(State.currentSeg + n); }
function stepBack(n = 1)    { stopPlay(); goToSegment(State.currentSeg - n); }
function resetSim()         { stopPlay(); goToSegment(0); }
